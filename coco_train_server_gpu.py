...
def train(lr,epochs,train_loader,val_loader,name_model,momentum=0.9,weight_decay=1e-3):
    ...
    for training_ind in [int(sys.argv[2])]:
...
def main():
    with open(f'./artifacts/coco_data_{sys.argv[1]}_train.pkl', 'rb') as f:
        [x_train, y_train, _, _] = pickle.load(f)
    
    with open(f'./artifacts/coco_data_{sys.argv[1]}_val.pkl', 'rb') as f:
    ...
    train(lr,epochs,train_loader,val_loader,f'./models/coco_{sys.argv[1]}',momentum=momentum,weight_decay=weight_decay)
