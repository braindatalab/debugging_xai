...
def print_AUC(loader, model_ind=0, split=0):
    ...
    cnn_conf=load_trained(f'./models/cnn_confounder_variable_{split}_{model_ind}.pt').to(DEVICE).eval()
    cnn_sup=load_trained(f'./models/cnn_suppressor_variable_{split}_{model_ind}.pt').to(DEVICE).eval()
    cnn_no=load_trained(f'./models/cnn_no_watermark_variable_{split}_{model_ind}.pt').to(DEVICE).eval()
...
for split in [sys.argv[1]]:
    for i in range(5):
        ...
        with open(f'./artifacts/split_{split}_confounder_variable_test.pkl', 'rb') as f:
            confounder_test, labels_test_conf, _, _ = pickle.load(f)
            confounder_test = [[rescale_values(x,1,0).transpose(2,0,1),labels_test_conf.flatten()[i]] for i,x in enumerate(confounder_test)]

        with open(f'./artifacts/split_{split}_suppressor_variable_test.pkl', 'rb') as f:
            suppressor_test, labels_test_sup, _, _ = pickle.load(f)
        ...
        with open(f'./artifacts/split_{split}_no_watermark_variable_test.pkl', 'rb') as f:
            no_mark_test, labels_test_no, _, _ = pickle.load(f)
...
with open(f'./aurocs/auroc_results_split_variable_{split}.pickle', 'wb') as f:
    pickle.dump([confounder_data_results, suppressor_data_results, no_mark_data_results], f)

with open(f'./aurocs/auroc_conf_variable_{split}.pickle', 'wb') as f:
    pickle.dump([conf_conf, conf_sup, conf_no], f)    

with open(f'./aurocs/auroc_sup_variable_{split}.pickle', 'wb') as f:
    pickle.dump([sup_conf, sup_sup, sup_no], f)

with open(f'./aurocs/auroc_no_variable_{split}.pickle', 'wb') as f:
    pickle.dump([no_conf, no_sup, no_no], f)

with open(f'./aurocs/conf_labels_variable_{split}.pickle', 'wb') as f:
    pickle.dump(conf_labels, f)

with open(f'./aurocs/sup_labels_variable_{split}.pickle', 'wb') as f:
    pickle.dump(sup_labels, f)

with open(f'./aurocs/no_labels_variable_{split}.pickle', 'wb') as f:
...
plt.tight_layout()
plt.savefig(f'./figures/roc_curves_variable_{split}.png', bbox_inches='tight')
