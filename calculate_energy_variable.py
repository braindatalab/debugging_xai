...
with open(f'./artifacts/split_{split}_no_watermark_variable_test.pkl', 'rb') as f:
...
with open(f'./artifacts/split_{split}_all_watermark_variable_test.pkl', 'rb') as f:
...
def energy(att, mask):
    mask[mask>0]=1
    image_size=att.shape[0]*att.shape[1]
    watermark_size=np.sum(mask)
    
    watermark_att=att*mask
    watermark_energy=np.sum(watermark_att)
...
for model_ind in [int(model_ind)]:
    
    model_conf=load_trained(f'./models/cnn_confounder_variable_{split}_{model_ind}.pt').eval().to(DEVICE)
    model_sup=load_trained(f'./models/cnn_suppressor_variable_{split}_{model_ind}.pt').eval().to(DEVICE)
    model_no=load_trained(f'./models/cnn_no_watermark_variable_{split}_{model_ind}.pt').eval().to(DEVICE)
    ...
    for i in range(len(watermark_dataset)):
        ...
        for method in list(energy_water_conf.keys()):
            energy_water_conf[method].append(energy(a_conf_w[method], w_mask))
            energy_water_sup[method].append(energy(a_sup_w[method], w_mask))
            energy_water_no[method].append(energy(a_no_w[method], w_mask))

            energy_no_water_conf[method].append(energy(a_conf_nw[method], w_mask))
        ...
        x_wm = energy(np.dot(w_image.copy().transpose(1,2,0)[...,:3], rgb_weights), w_mask)
        x_no = energy(np.dot(nw_image.copy().transpose(1,2,0)[...,:3], rgb_weights), w_mask)
        ...
        lapl_wm = energy(np.abs(laplace(img_r)) + np.abs(laplace(img_g)) + np.abs(laplace(img_b)), w_mask)
        sob_wm = energy(np.abs(sobel(img_r)) + np.abs(sobel(img_g)) + np.abs(sobel(img_b)), w_mask)
        ...
        lapl_no = energy(np.abs(laplace(img_r)) + np.abs(laplace(img_g)) + np.abs(laplace(img_b)), w_mask)
        sob_no = energy(np.abs(sobel(img_r)) + np.abs(sobel(img_g)) + np.abs(sobel(img_b)), w_mask)
...
with open(f'./energies/energy_water_conf_pred_variable_{split}_{model_ind}.pickle', 'wb') as f:
    pickle.dump(energy_water_conf, f)    
with open(f'./energies/energy_water_sup_pred_variable_{split}_{model_ind}.pickle', 'wb') as f:
    pickle.dump(energy_water_sup, f)    
with open(f'./energies/energy_water_no_pred_variable_{split}_{model_ind}.pickle', 'wb') as f:
    pickle.dump(energy_water_no, f)    
    
with open(f'./energies/energy_no_water_conf_pred_variable_{split}_{model_ind}.pickle', 'wb') as f:
    pickle.dump(energy_no_water_conf, f)    
with open(f'./energies/energy_no_water_sup_pred_variable_{split}_{model_ind}.pickle', 'wb') as f:
