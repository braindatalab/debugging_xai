...
def generate_data(inds, beta_norm, beta_dark, beta_light, beta_conf, manip_sup, manip_conf, class_name='animal', im_size=224):
    ...
    for i, ind in enumerate(inds):
        ...
        if ind in manip_conf:
            ...
            if beta_conf == beta_dark:
                manips_conf.append('dark')
            else:
                manips_conf.append('light')
        else:
...
for i in [int(sys.argv[1])]:
    ...
    for split, split_results in {'train': [train_inds, 0.7], 'val': [val_inds, 0.15], 'test': [test_inds, 0.15]}.items():
        ...
        for j, (key, value) in enumerate(animal_no_intersections.items()):
            ...
            data_norm_half, data_conf_half, data_sup_half, masks_half, manips_norm_half, manips_conf_half, manips_sup_half = generate_data(value[split_results[0]][:n_per_class_split], beta_norm, beta_dark, beta_light, beta_conf, manip_half, manip_half, key)
