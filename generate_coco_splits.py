...
for i in [int(sys.argv[1])]:
    ...
    # This is terrible naming, sorry. split here = train/val/test; sys.argv[1] is for the split (shuffling of data)
    for split, split_results in {'train': [train_inds, 0.7], 'val': [val_inds, 0.15], 'test': [test_inds, 0.15]}.items():
        ...
        with open(f'./artifacts/split_{i}_coco_data_norm_{split}.pkl', 'wb') as f:
            pkl.dump([data_norm, labels, masks, manip], f)

        with open(f'./artifacts/split_{i}_coco_data_conf_{split}.pkl', 'wb') as f:
            pkl.dump([data_conf, labels, masks, manip], f)

        with open(f'./artifacts/split_{i}_coco_data_sup_{split}.pkl', 'wb') as f:
