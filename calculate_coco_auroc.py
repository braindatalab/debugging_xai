...
with open(f'./artifacts/coco_data_conf_test.pkl', 'rb') as f:
    [x_test, y_test, _] = pickle.load(f)
...
with open(f'./artifacts/coco_data_sup_test.pkl', 'rb') as f:
    [x_test, y_test, _] = pickle.load(f)
...
for i in [int(sys.argv[1])]:
    ...
    print(f'MODEL {i}')
    conf_scores, [conf_model_conf_test, sup_model_conf_test, norm_model_conf_test, conf_true_labels] = print_AUC(confounder_loader, i)
    print()
    print('suppressor data:')
    sup_scores, [conf_model_sup_test, sup_model_sup_test, norm_model_sup_test, sup_true_labels] = print_AUC(suppressor_loader, i)

    print()
    print('norm data:')
    norm_scores, [conf_model_norm_test, sup_model_norm_test, norm_model_norm_test, norm_true_labels] = print_AUC(norm_loader, i)
...
with open(f'auroc_results_coco_{split}.pickle', 'wb') as f:
    pickle.dump([confounder_data_results, suppressor_data_results, norm_data_results], f)

with open(f'auroc_conf_coco_{split}.pickle', 'wb') as f:
    pickle.dump([conf_conf, conf_sup, conf_no], f)    

with open(f'auroc_sup_coco_{split}.pickle', 'wb') as f:
    pickle.dump([sup_conf, sup_sup, sup_no], f)

with open(f'auroc_norm_coco_{split}.pickle', 'wb') as f:
    pickle.dump([norm_conf, norm_sup, norm_no], f)

with open(f'conf_labels_coco_{split}.pickle', 'wb') as f:
    pickle.dump(conf_labels, f)

with open(f'sup_labels_coco_{split}.pickle', 'wb') as f:
    pickle.dump(sup_labels, f)

with open(f'norm_labels_coco_{split}.pickle', 'wb') as f:
