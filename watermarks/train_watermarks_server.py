...
def load_split(artifacts_dir, split_index, base, subset, scale, position, invert):
    ...
    X = np.transpose(to01(data), (0, 3, 1, 2))        # NCHW
    y = np.asarray(labels).reshape(-1).astype(np.int64)
    return X, y, path

def make_loaders(artifacts_dir, split_index, base, scale, position, invert, batch_size, num_workers=4, pin_memory=True):
    ...
    tr_loader = DataLoader(tr, batch_size=batch_size, shuffle=True,  num_workers=num_workers, pin_memory=pin_memory)
...
def train_one_seed(seed, base, tr_loader, va_loader, lr, epochs, wd, momentum, model_dir, tag, device):
    set_seed(seed)
    torch.backends.cudnn.benchmark = True  # fixed 128x128
    model = Net().to(device)
    ...
    for ep in range(1, epochs+1):
        ...
        v_loss, v_acc, v_roc = evaluate(model, va_loader, criterion, device)
...
def main(argv=None):
    ...
    ap.add_argument("--seeds", type=int, nargs="*", default=[0,1,2,3,4])   # indexes into SEEDS
    ap.add_argument("--gpu", type=int, default=0, help="CUDA device index to use")
